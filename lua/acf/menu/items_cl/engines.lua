...
local HeatText = [[
	Peak Heat : %s kW/s @ %s RPM]]
local TankSize    = Vector()

local function UpdateAdvEngineStats(Label, Data)
	local PeakHeat = Data.PeakHeatEnergy
	local PeakPowerRPM = Data.PeakPowerRPM
	
	local HeatLabelText = HeatText:format(PeakHeat, PeakPowerRPM) 
	Label:SetText(HeatLabelText)
end
...
local function CreateMenu(Menu)
	...
	local AdvEngine = Menu:AddCheckBox("Advanced Engine")
	local PeakHeat = EngineBase:AddLabel()
	PeakHeat:SetVisible(false)
	...
	function EngineList:OnSelect(Index, _, Data)
		...
		if Data.AdvancedEngine then
			UpdateAdvEngineStats(PeakHeat, Data)
		end
	...
	function AdvEngine:OnChange(bVal)
		local Adv = EngineClass.Selected.Items
		for i,v in ipairs(Adv) do 
			if(bVal&&not v.AdvancedEngine)then
				PeakHeat:SetVisible(true)
				v.AdvancedEngine = true
				ACF.SetClientData("AdvancedEngine", true)
				local Data = EngineList.Selected
				UpdateAdvEngineStats(PeakHeat, Data)
			else
				PeakHeat:SetVisible(false)
				v.AdvancedEngine = false
				ACF.SetClientData("AdvancedEngine", false)
			end
		end
	end

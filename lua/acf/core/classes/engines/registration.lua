...
local function AddAdvancedData(Engine)
	local Adv = Engine.AdvancedEngine
	local HeatEfficiency = Engine.HeatEfficiency
	
	if not Adv then 
		Engine.AdvancedEngine = false
	end
	if not HeatEfficiency then
		Engine.HeatEfficiency = 0.3
	end
	ACF.AddAdvancedEngineData(Engine)
end
	...
	hook.Add("ACF_OnLoadAddon", "ACF Engine Performance", function()
		...
		for Name in pairs(Engines.GetEntries()) do
			for _, Engine in pairs(Engines.GetItemEntries(Name)) do
				...
				AddAdvancedData(Engine)--hijacked
			end
		end

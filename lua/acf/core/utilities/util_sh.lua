...
do -- Mobility functions
	...
	function ACF.AddRadiatorData(Class)
		local HeatDisipationCoeff = Class.HeatDisopationCoeff
		
	end
	
	function ACF.AddAdvancedEngineData(Class)
		local Adv = Class.AdvancedEngine
		local PeakHeatEnergy = Class.PeakHeatEnergy
		
		if not PeakHeatEnergy then
			PeakHeatEnergy = Class.PeakPower*(1/0.3-1)
		end
		
		Class.PeakHeatEnergy = math.Round(PeakHeatEnergy)
	end

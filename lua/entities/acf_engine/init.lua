...
do
	...
	ACF.RegisterClassLink("acf_engine", "acf_radiator", function(Engine, Target)
		if Engine.AdvancedEngine then
			if Engine.Radiators[Target] then return false, "This engine is already linked to this radiator!" end
			if Target.Engines[Engine] then return false, "This engine is already linked to this radiator!" end
			--add check for air cooling?
			if Target.NoLinks then return false, "This radiator doesn't allow linking." end
			if Engine:GetPos():DistToSqr(Target:GetPos()) > MaxDistance then return false, "This radiator is too far away from this engine." end
			...
			Engine:UpdateOverlay()
			Target:UpdateOverlay()
			Engine.Coolant = Engine.Coolant+Target.Coolant
			Engine.Capacity = Engine.Capacity+Target.Capacity
			return true, "Engine linked successfully!"
		else
			return false, "This engine is not an Advanced Engine."
		end
	end)

	ACF.RegisterClassUnlink("acf_engine", "acf_radiator", function(Engine, Target)
		if Engine.AdvancedEngine then
			if Engine.Radiators[Target] or Target.Engines[Engine] then
				if Engine.Radiator == Target then
					Engine.Radiator = next(Engine.Radiators, Target)
				end
...
local function GetNextRadiator(Engine)
	...
	if not Radiators then return end
...
local function CheckDistantRadiators(Engine)
	...
	for Radiator in pairs(Engine.Radiators) do
		if EnginePos:DistToSqr(Radiator:GetPos()) > MaxDistance then
			local Sound = UnlinkSound:format(math.random(1, 3))

			Sounds.SendSound(Engine, Sound, 70, 100, 1)
			Sounds.SendSound(Radiator, Sound, 70, 100, 1)

			Engine:Unlink(Radiator)
		end
	end
end
...
do -- Spawn and Update functions
	...
	local function UpdateEngine(Entity, Data, Class, Engine, Type)
		...
		Entity.AdvancedEngine	= Data.AdvancedEngine
		if Entity.AdvancedEngine then
			Entity.HeatEfficiency = Engine.HeatEfficiency
			Entity.PeakHeat = Entity.PeakPower*(1/Entity.HeatEfficiency-1)
			Entity.HeatProduced = 0
			Entity.AmbientTemp = 25
			Entity.Temperature = Entity.AmbientTemp
			Entity.CoolantTemp = Entity.AmbientTemp
			Entity.WaterJacketLength = 0.2
			Entity.Mass = Engine.Mass
			Entity.SurfaceArea = Entity:GetPhysicsObject():GetSurfaceArea()*2.54/1000
			
			Entity.FlowMax = 1 --L/sec
			--heat loss due to radiation
			
			TimerCreate("ACF Radiation"..Entity:EntIndex(), 1, 0, function()
				if not IsValid(Entity) then return end
				local DeltaTime = 0.1
				local Temperature = Round(Entity.Temperature,3)
				local CoolantTemp = Round(Entity.CoolantTemp,3)
				if Temperature <= 25 then return end
				
				local Health = Entity.ACF.Health
				local MaxHealth = Entity.ACF.MaxHealth
				local Percent = Health/MaxHealth
				if Temperature > 25 then
					TemperatureLoss = 0.1*Entity.SurfaceArea/3*5.67*10^-8*(Temperature^4-25^4)*DeltaTime
					Temperature = Temperature-TemperatureLoss
					CoolantTemperatureLoss = 0.3*Entity.SurfaceArea/3*5.67*10^-8*(CoolantTemp^4-25^4)*DeltaTime
					CoolantTemp = CoolantTemp-CoolantTemperatureLoss
					Entity.Temperature = Temperature
					Entity.CoolantTemp = CoolantTemp
					Entity:UpdateOutputs(Entity:GetTable())
					if Temperature > 110 then
						Entity.ACF.Health = Health*0.95
					end
					if Percent < 0.1 and Entity.Active then
						if Percent < 0.05 and Entity.Active then
							Entity:Remove()
						else
							SetActive(Entity, false, Entity:GetTable())
						end
					end
				end
				
				local Radiators = Entity.Radiators
				local RC = table.Count( Radiators )
				if RC ~= 0 then
					for Rad, Int in pairs(Radiators) do
						if Entity.CoolantTemp>30 and Entity.Temperature>=30 and not Entity.Active then
							Entity.CoolantTemp = Rad:IntakeCoolant(0.1, CoolantTemp, Rad:GetTable(), 0.25, Entity.Active)
						end
					end
				end
			end)
		else
			Entity.Radiators = nil
		end
	...
	function MakeACF_Engine(Player, Pos, Angle, Data)
		...
		if Data.AdvancedEngine then
			Engine.AdvancedEngine = true
			Entity.Radiators = {}
		else
			Engine.AdvancedEngine = false
			Entity.Radiators = nil
		end
	...
	function ENT:Update(Data)
		...
		if self.AdvancedEngine ~= Data.AdvancedEngine then
			self.AdvancedEngine = Data.AdvancedEngine
		end
...
function ENT:UpdateOutputs(SelfTbl)
	...
	local Temperature = Round(SelfTbl.Temperature,3)
	local CoolantTemp = Round(SelfTbl.CoolantTemp,3)
	...
	if SelfTbl.Temperature ~= Temperature then
		SelfTbl.Temperature = Temperature
		WireLib.TriggerOutput(self, "Temperature", Temperature)
	end
	if SelfTbl.CoolantTemp ~= CoolantTemp then
		SelfTbl.CoolantTemp = CoolantTemp
		WireLib.TriggerOutput(self, "CoolantTemp", CoolantTemp)
	end
end
...
function ENT:UpdateOverlayText()
	...
	local Advanced = self.AdvancedEngine
	if Advanced then
		Text = "%s\n\n%s\nPower: %s kW / %s hp\nTorque: %s Nm / %s ft-lb\nPowerband: %s - %s RPM\nRedline: %s RPM\nAdvanced: %s"
	end

	return Text:format(State, Name, Power, PowerFt, Torque, TorqueFt, PowerbandMin, PowerbandMax, Redline, Advanced)
end
...
function ENT:CalcMassRatio(SelfTbl)
	...
	if SelfTbl.LastTotalMass ~= TotalMass then
		...
		Contraption.TotalMass = TotalMass
...
function ENT:CalcRPM(SelfTbl)
	...
	if SelfTbl.AdvancedEngine then
		local Efficiency 		= SelfTbl.HeatEfficiency
		local Temperature 		= SelfTbl.Temperature
		local CoolantTemp 		= SelfTbl.CoolantTemp
		local Power 			= (Torque*FlyRPM)/9549.3*DeltaTime
		local WaterJacketLength	= SelfTbl.WaterJacketLength
		local HeatProduced 		= Power*(1-Efficiency)*DeltaTime
		local HeatLossExhaust			= 0--HeatProduced/3-- 1/3 loss to exhaust
		local FlowRate 				= (SelfTbl.FlyRPM/SelfTbl.LimitRPM)*SelfTbl.FlowMax
		local NetHeat 					= HeatProduced-HeatLossExhaust
		local TempDelta 			= NetHeat/((SelfTbl.Mass*0.45)*DeltaTime)--0.3 is the heat loss to exhaust,7860 density of iron, 0.46 is the specific heat of cast iron
		local CoolantTempDelta		= NetHeat/((FlowRate*1.11*3.14)*DeltaTime)
		local CoolantTempDeltaFromEngine = (0.6*0.2*(Temperature-CoolantTemp))/(FlowRate*1.11*3.14)
		
		SelfTbl.Temperature = Clamp(Temperature+TempDelta,25,500)
		self:UpdateOutputs(SelfTbl)
		local Radiators = SelfTbl.Radiators
		local RC = table.Count( Radiators )
		if RC ~= 0 then
			for Rad, Int in pairs(Radiators) do
				if(SelfTbl.CoolantTemp>30 and SelfTbl.Temperature>=30) then 
					local Percent = SelfTbl.Coolant/SelfTbl.Capacity
					if Percent <= 0.01 then Percent = 0 end
					SelfTbl.CoolantTemp = Clamp(Round(Rad:IntakeCoolant(FlowRate*Percent, CoolantTemp, Rad:GetTable(), DeltaTime, SelfTbl.Active),3),-100,100)
				end
				SelfTbl.Coolant = SelfTbl.Coolant + Rad:GetTable().Coolant
				SelfTbl.Capacity = SelfTbl.Capacity + Rad:GetTable().Capacity
			end
		end
		SelfTbl.CoolantTemp 	= Clamp(CoolantTemp+CoolantTempDelta+CoolantTempDeltaFromEngine,25,500)
		--print("Engine Coolant "..SelfTbl.CoolantTemp.." Delta "..CoolantTempDelta..", "..CoolantTempDeltaFromEngine)
	end
